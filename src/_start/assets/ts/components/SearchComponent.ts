...
class SearchComponent {
  ...
  constructor(_element: HTMLElement, options: SearchOptions) {
    ...
    if (this.mode === 'menu') {
      const attr = this.element.getAttribute('data-hb-menu-target');
      if (attr) {
      ...
      this.menu = this.element.closest('[data-hb-menu]');
      if (this.menu) {
  ...
  private _getOption = (name: string) => {
    const attr = this.element.getAttribute('data-hb-search-' + name);
    if (attr) {
