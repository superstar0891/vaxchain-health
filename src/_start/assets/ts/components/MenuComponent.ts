...
class MenuComponent {
  ...
  private _setTriggerElement = () => {
    const target = document.querySelector(
      `[data-hb-menu-target="#${this.element.getAttribute("id")}"`
    );

    if (target) {
      this.triggerElement = target as HTMLElement;
    } else if (this.element.closest("[data-hb-menu-trigger]")) {
      this.triggerElement = this.element.closest(
        "[data-hb-menu-trigger]"
      ) as HTMLElement;
    } else if (
  ...
  private _getItemOption = (item: HTMLElement, name: string) => {
    let value = null;
    if (item && item.hasAttribute("data-hb-menu-" + name)) {
      const attr = item.getAttribute("data-hb-menu-" + name) || "";
  ...
  private _getItemElement = (_element: HTMLElement) => {
    ...
    if (_element.hasAttribute("data-hb-menu-trigger")) {
      return _element;
    }
    ...
    const item = _element.closest(".menu-item[data-hb-menu-trigger]");
    if (item) {
      return item;
    }
  ...
  private _getItemParentElement = (item: HTMLElement) => {
    ...
    const parentItem = sub.closest(".menu-item[data-hb-menu-trigger]");
    if (sub && parentItem) {
      return parentItem;
    }
  ...
  private _getItemChildElement = (item: HTMLElement) => {
    ...
    if (selector) {
      //element = selector.querySelector('.show.menu-item[data-hb-menu-trigger]');
      const element = selector.querySelector(
        ".menu-item[data-hb-menu-trigger]"
      );
      if (element) {
        return element;
      }
  ...
  private _hasItemSub = (item: HTMLElement) => {
    return (
      ...
      item.hasAttribute("data-hb-menu-trigger")
    );
  };
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    const items = document.querySelectorAll(
      ".show.menu-dropdown[data-hb-menu-trigger]"
    );
  ...
  public static initGlobalHandlers = () => {
    ...
    DOMEventHandlerUtil.on(
      document.body,
      ".menu-item[data-hb-menu-trigger] > .menu-link, [data-hb-menu-trigger]:not(.menu-item)",
      "click",
    ...
    DOMEventHandlerUtil.on(
      document.body,
      ".menu-item:not([data-hb-menu-trigger]) > .menu-link",
      "click",
      function (this: HTMLElement, e: Event) {
        e.stopPropagation();
    ...
    window.addEventListener("resize", () => {
      ...
      throttle(
        ...
        () => {
          ...
          const elements = document.querySelectorAll('[data-hb-menu="true"]');
          elements.forEach((el) => {
  ...
  public static bootstrap = () => {
    ...
    MenuComponent.createInstances('[data-hb-menu="true"]');
  };
