...
class StepperComponent {
  ...
  private getStepContent = (index: number) => {
    const content = this.element.querySelectorAll(
      '[data-hb-stepper-element="content"]'
    );
    if (!content) {
      return false;
    }
  ...
  private refreshUI = () => {
    ...
    const elements = this.element.querySelectorAll(
      '[data-hb-stepper-element="nav"], [data-hb-stepper-element="content"], [data-hb-stepper-element="info"]'
    );
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (
          ...
          element.getAttribute("data-hb-stepper-element") === "content"
        ) {
  ...
  public static bootstrap(attr: string = "[data-hb-stepper]") {
